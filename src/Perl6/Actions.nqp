...
class Perl6::Actions is HLL::Actions does STDActions {
    ...
    method comp_unit($/) {
        ...
        $compunit.ann('UNIT',      $unit);
        $compunit.ann('GLOBALish', $*GLOBALish);
        $compunit.ann('W',         $*W);
        ...
        if $<EXPR> {
            ...
            if $ml {
                ...
                if ~$ml<sym> eq 'given' {
                    ...
                    my $sinkee := $past[0];
                    $past.ann('statement_level', -> { $sinkee.name('sink') });
                }
                else {
        ...
        if $<blockoid><you_are_here> {
            ...
            if $block.ann('placeholder_sig') && $<signature> {
                $*W.throw($/, ['X', 'Signature', 'Placeholder'],
                    placeholder => $block.ann('placeholder_sig')[0].ann('placeholder'),
                );
            }
            ...
            $ref.ann('uninstall_if_immediately_used', $uninst);
            make $ref;
        }
    }
    ...
    method blockoid($/) {
        if $<statementlist> {
            ...
            $BLOCK.push($past);
            $BLOCK.node($/);
            $BLOCK.ann('statementlist', $<statementlist>.ast);
...
#            }
    ...
    method statement_prefix:sym<try>($/) {
        my $block := $<blorst>.ast;
        my $past;
        if (my $pastblock := $block.ann('past_block')).ann('handlers') && $pastblock.ann('handlers')<CATCH> {
    ...
    method package_def($/) {
        ...
        if $*PKGDECL eq 'role' {
            ...
            $throwaway_block_past.ann('outer', $block);
    ...
    method routine_def($/) {
        ...
        $closure.ann('sink_past', QAST::Op.new( :op('null') ));
        make $closure;
    }
    ...
    method circumfix:sym<{ }>($/) {
        ...
        if $is_hash && $past.ann('past_block').arity == 0 {
            ...
            $past.ann('bare_block', QAST::Op.new(
                :op('call'),
...
#            say('dec_number exponent: ' ~ ~$e.ast);
    ...
    method quote:sym</ />($/) {
        ...
        # Return closure if not in sink context.
        my $closure := block_closure($coderef);
        $closure.ann('sink_past', QAST::Op.new( :op<callmethod>, :name<Bool>, $closure));
        make $closure;
    }
    ...
    method quote:sym<rx>($/) {
        ...
        $past.ann('sink_past', QAST::Op.new(:op<callmethod>, :name<Bool>, $past));
        make $past;
    }
    method quote:sym<m>($/) {
        my $block := QAST::Block.new(QAST::Stmts.new, QAST::Stmts.new, :node($/));
    ...
    sub add_placeholder_parameter($/, $sigil, $ident, :$named, :$pos_slurpy, :$named_slurpy, :$full_name) {
        ...
        my @params := $block.ann('placeholder_sig') || ($block.ann('placeholder_sig', []));
    ...
    sub reference_to_code_object($code_obj, $past_block) {
        ...
        $ref.ann('past_block', $past_block);
        $ref.ann('code_object', $code_obj);
        $ref
    }
    ...
    sub block_closure($code) {
        ...
        $closure.ann('past_block', $code.ann('past_block'));
        $closure.ann('code_object', $code.ann('code_object'));
        $closure
    }
    ...
    sub capture_or_parcel($args, $name) {
        if $name eq 'return' {
            ...
            for @($args) {
                $parcel.push($_.ann('before_promotion') ?? $_.ann('before_promotion') !! $_);
            }
            $parcel
        }
        else {
    ...
    sub whatever_curry($/, $past, $upto_arity) {
        ...
        if $whatevers {
            ...
            $past.arity(+@params);
            if $was_chain {
                $past.ann('chain_past' , @old_args);
                $past.ann('chain_args' , @params);
                $past.ann('chain_name' , $was_chain);
                $past.ann('chain_block', $block);
            }
        }
        $past
    }
